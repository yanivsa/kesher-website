...
const postForm = (path:string, values:Record<string,string>, cookie:string) => nnnFetch(path,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8',cookie:`ok=${cookie}`,'user-agent':'Kesher-Saharoni-NNN-Setup/1.0'},body:formBody(values)});
...
async function requestLoginLink(){
  const r=await nnnFetch('/f',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8','user-agent':'Kesher-Saharoni-NNN-Setup/1.0'},body:formBody({email:'yanivashdod@gmail.com'})});
...
async function bootstrap(token:string){
  ...
  const welcome=await nnnFetch(`/e?t=${encodeURIComponent(token)}`,{headers:{'user-agent':'Kesher-Saharoni-NNN-Setup/1.0'}});
  const personId=extractInputValue(await welcome.text(),'i');
  if(!personId||!/^\d+$/.test(personId)) return json({ok:false,error:'could not extract person id',status:welcome.status},502);
  const login=await nnnFetch('/e',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8','user-agent':'Kesher-Saharoni-NNN-Setup/1.0'},body:formBody({t:token,i:personId})});
  const cookie=extractOkCookie(login);
  if(!cookie) return json({ok:false,error:'login did not return auth cookie',status:login.status},502);
  const where=await postForm('/where',{city:'Ashdod',state:'',country:'IL'},cookie);
  const urlsHtml=await (await nnnFetch('/urls',{headers:{cookie:`ok=${cookie}`,'user-agent':'Kesher-Saharoni-NNN-Setup/1.0'}})).text();
  ...
  const profileHtml=await (await nnnFetch('/profile',{headers:{cookie:`ok=${cookie}`,'user-agent':'Kesher-Saharoni-NNN-Setup/1.0'}})).text();
  const photoHtml=await (await nnnFetch('/photo',{headers:{cookie:`ok=${cookie}`,'user-agent':'Kesher-Saharoni-NNN-Setup/1.0'}})).text();
