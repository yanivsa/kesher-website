...
2. **`/services/gifted-parenting` (עמוד שירות / Hub — שודרג בענף הפיילוט):**
   - הוספת סכמת `FAQPage` עשירה בנתונים מובנים בהתאמה מילולית מלאה (100% Verbatim Match) לטקסט הגלוי בעמוד.
   - ניסוח ניטרלי ומשלים בשאלות הנפוצות: הבהרת ההבחנה בין הדרכת הורים (מיקוד באקוסיסטם המשפחתי, שגרה וכלים להורים) לבין טיפול רגשי לילד (מרחב אישי לעיבוד רגשי), ללא טענות יעילות השוואתיות.
