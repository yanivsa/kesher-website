...
**סטטוס:** תשתית ופיילוט פרוסים בייצור; תקופת מדידה החלה ב-16 בספטמבר 2026  
...
1. **הושלם — תשתית AEO מוזגה ונפרסה:** PR #834 מוזג ל-`main` והפריסה אומתה בייצור.
2. **הושלם — פיילוט AEO מוזג ונפרס:** PR #835 מוזג ל-`main` וחמש כתובות הפיילוט אומתו בייצור.
3. **קלט מומחה משירה:** להשלים את השאלות ב-`docs/aeo/expert-input-needed.md` לפני הרחבה; המעקב מנוהל ב-GitHub Issue #845.
4. **מדידת 30 יום:** לבצע ב-16 באוקטובר 2026 לפי `docs/aeo/experiments.csv` ו-`docs/aeo/monitoring-prompts.csv`; המעקב מנוהל ב-GitHub Issue #844.
5. **עדכון כרטיסי ישות חיצוניים:** להחליט בנפרד על אינדקסים ישראליים ו-Google Business Profile לפי `docs/aeo/external-entity-plan.md`.
6. **אסטרטגיית וידאו:** להחליט בנפרד על הפקת 3-5 סרטוני תשובות ישירות קצרים לשירה בערוץ YouTube.
7. **Jules rollout:** לא להתחיל rollout המוני לפני סגירת Issue #844 והשלמת scale gate; לאחר מכן לפעול במנות של 5-8 URLs לפי `docs/aeo/jules-rollout-plan.md`.
