...
4. Metadata: compare title, description and every tag with source and transcript. Reject unsupported metadata, default/generic metadata or English. The description MUST contain three separate full URL lines: the exact canonical article URL from the source, `https://kesher.saharoni.com`, and `https://kesher.saharoni.com/appointment`. A domain appearing only inside the article URL does not satisfy the standalone site-link requirement. Separately confirm that `generation_prompt` explicitly requests a female Hebrew voice (`השתמש בקול של אישה ישראלית, חם, טבעי, ברור ומקצועי לכל אורך הקריינות.`).
