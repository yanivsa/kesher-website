#!/usr/bin/env python3
"""Deterministic per-video motion + enhancement plan for Kesher Overview.

The existing pixel-derived motion plan remains intact. A shared enhancement
contract is added alongside it so Remotion can safely use approved optional
assets while retaining a source-only fallback.
"""
...
def analyze_frame_saliency(pixel_bytes: bytes) -> dict[str, float]:
    """Compute spatial saliency center and detail energy."""
    ...
    return {
        "originX": round(max(20.0, min(80.0, cx_norm)), 1),
        "originY": round(max(20.0, min(80.0, cy_norm)), 1),
        "edge_density": round(diff_sum / (w * h), 2),
    }


def detect_scene_cuts(frames: list[dict[str, Any]]) -> list[int]:
    """Detect scene cut frame indices using mean pixel differences."""
    if len(frames) < 2:
        return [0, len(frames)]
...
def _timeline_type(transform_type: str) -> str:
    if transform_type in {"pan_right", "pan_left"}:
        return "pan"
    if transform_type == "tracked_reframe":
        return "reframe"
    return "push_in"
...
def generate_motion_plan(
    ...
    """Produce the legacy motion fields plus the shared enhancement edit plan."""
    ...
    for c_idx in range(len(cuts) - 1):
        ...
        seg_frames = sampled_frames[sample_start:sample_end] or sampled_frames[sample_start:sample_start + 1]
        ...
        ttype = transform_types[int(avg_origin_x + avg_origin_y + avg_edge_density) % len(transform_types)]
    ...
    base_timeline = [
        {
            "start": round(segment["startFrame"] / calc_fps, 3),
            "end": round(segment["endFrame"] / calc_fps, 3),
            "type": _timeline_type(segment["transformType"]),
            "intent": "source-derived camera motion from pixel saliency",
        }
...
if __name__ == "__main__":
    ...
    print(json.dumps(generate_motion_plan(inp, outp), ensure_ascii=False, indent=2))
