...
MARKER_RE = re.compile(r"OPENCLAW_[A-Z0-9_]+=[^\r\n]*")
...
def enable_run_command(compute, inst) -> None:
    ...
    print("OCI_LOCAL_PROOF_PLUGIN_REQUESTED=true", flush=True)
    ...
    while time.time() < deadline:
        try:
            ...
            if transient:
                ...
                raise
        if status != last:
            print(f"OCI_LOCAL_PROOF_PLUGIN_STATUS={status}", flush=True)
            last = status
        if status == "RUNNING":
            time.sleep(20)
            return
...
def safe_markers(text: str) -> list[str]:
    return list(
        dict.fromkeys(m.group(0).strip()[-700:] for m in MARKER_RE.finditer(text))
    )[-80:]
...
def main() -> int:
    ...
    details = oci.compute_instance_agent.models.CreateInstanceAgentCommandDetails(
        ...
        display_name="openclaw-local-proof",
    ...
    print(f"OCI_LOCAL_PROOF_AGENT_COMMAND_ID={created.id}", flush=True)
    ...
    while time.time() < deadline:
        ...
        if key != last:
            print(
                f"OCI_LOCAL_PROOF_AGENT_STATE={lifecycle} delivery={delivery}",
                flush=True,
            )
            last = key
        ...
        if lifecycle in TERMINAL or exit_code is not None:
            ...
            failures = [
                m
                for m in markers
                if m.startswith("OPENCLAW_LOCAL_PROOF_FAILED=")
                or m.startswith("OPENCLAW_FINALIZE_FAILED=")
            ]
            if failures:
            ...
            if lifecycle != "SUCCEEDED" or exit_code != 0 or missing:
                raise RuntimeError(
                    "OPENCLAW_LOCAL_PROOF_FAILED "
                    f"state={lifecycle} exit={exit_code} "
                ...
                json.dumps(
                    {
                        ...
                        "gateway_port": 18789,
                        "proof": "preserved-boot-disk-local-finalizer-log-and-ready-file",
                    }
                )
            )
            print("OPENCLAW_LOCAL_PROOF_VERIFIED=true", flush=True)
            return 0
        time.sleep(5)

    raise TimeoutError("OPENCLAW_LOCAL_PROOF_WAIT_TIMEOUT")
