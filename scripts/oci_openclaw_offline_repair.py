...
def prepare(args) -> int:
    ...
    pubkey = Path(args.ssh_public_key_file).read_text().strip()
    helper = compute.launch_instance(
        oci.core.models.LaunchInstanceDetails(
            ...
            freeform_tags={
                "managed-by": "chatgpt",
                "purpose": "openclaw-offline-repair-helper",
            },
        )
    ).data
    ...
    for _ in range(36):
        ...
        time.sleep(5)
    if not helper_ip:
        raise RuntimeError("HELPER_PUBLIC_IP_NOT_ASSIGNED")
    log("OFFLINE_REPAIR_HELPER_RUNNING", helper_id=helper.id, public_ip=helper_ip)
    ...
    target = compute.launch_instance(
        oci.core.models.LaunchInstanceDetails(
            ...
            freeform_tags={
                ...
                "recovery": "offline-boot-volume-repair",
            },
        )
    ).data
