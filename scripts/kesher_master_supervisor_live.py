...
def new_supervisor_state() -> dict[str, Any]:
    return {"schema_version": SUPERVISOR_SCHEMA_VERSION, "incidents": {}, "commands": {}, "updated_at": None}
...
def _marker(report: dict[str, Any], strike: int, stage: str) -> str:
    return f"MASTER_ESCALATION fingerprint={incident_fingerprint(report)} strike={strike} stage={stage}"


def prepare_escalation(state: dict[str, Any], report: dict[str, Any], *, now: str, prior_action_terminal: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if isinstance(active, dict) and str(active.get("lifecycle") or "") in ACTIVE_LIFECYCLES:
        return result, {
            ...
            "reason": "terminal_claim_without_persisted_terminal_state" if prior_action_terminal else "active_command_not_terminal",
        }
    ...
    if command_id in commands:
        ...
        return result, {"stage": "WAIT", "executor": None, "execute_now": False, "command_id": command_id, "reason": "deterministic_command_already_exists"}
    commands[command_id] = {
    ...
    incident.setdefault("history", []).append({"at": now, "event": "command_issued", "command_id": command_id, "stage": stage})
    result["updated_at"] = now
    return result, {"stage": stage, "executor": executor, "execute_now": True, "command_id": command_id, "reason": "bounded_escalation"}


def mark_command_acknowledged(state: dict[str, Any], command_id: str, metadata: dict[str, Any], *, at: str) -> dict[str, Any]:
...
def mark_command_running(state: dict[str, Any], command_id: str, metadata: dict[str, Any], *, at: str) -> dict[str, Any]:
    ...
    result["commands"][command_id]["lifecycle"] = "running"
    result["commands"][command_id]["running_at"] = result["commands"][command_id].get("running_at") or at
    result["commands"][command_id]["updated_at"] = at
    result["updated_at"] = at
    return result
...
def mark_command_failed(state: dict[str, Any], command_id: str, reason: str, *, at: str) -> dict[str, Any]:
    ...
    if isinstance(incident, dict):
        ...
        incident.setdefault("history", []).append({"at": at, "event": "command_failed", "command_id": command_id, "reason": str(reason)})
    result["updated_at"] = at
    return result
...
def resolve_absent_incidents(state: dict[str, Any], report: dict[str, Any], *, at: str) -> tuple[dict[str, Any], bool]:
    ...
    current_fp = incident_fingerprint(report) if report.get("status") == "incident_detected" and report.get("incident_id") else None
    changed = False
...
def direct_dispatch_spec(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "rebuild_exact_overview" and item_id and slug:
        return {"workflow": "kesher-daily-video.yml", "inputs": {"operation": "rebuild", "rebuild_item_id": item_id, "target_slug": slug}}
    if action == "retry_exact_upload" and slug:
        return {"workflow": "kesher-daily-video.yml", "inputs": {"operation": "upload", "target_slug": slug}}
    if action == "continue_exact_short" and slug and content_hash and item_id:
        return {"workflow": "kesher-short-v4.yml", "inputs": {"operation": "derive", "derive_slug": slug, "derive_content_sha256": content_hash, "derive_long_item_id": item_id}}
...
class SupervisorStateStore:
    ...
    def save(self, state: dict[str, Any], *, expected_sha: str | None) -> str:
        return self.api.save_state_blob(SUPERVISOR_STATE_REF, SUPERVISOR_STATE_PATH, state, expected_sha)
...
class GitHubApi:
    ...
    def _request(self, method: str, url: str, body: dict[str, Any] | None = None, *, allow_404: bool = False, mutation: bool = False, cas_conflict: bool = False) -> Any:
        ...
        for attempt in range(attempts):
            req = urllib.request.Request(url, data=raw, method=method, headers={"Authorization": f"Bearer {self.token}", "Accept": "application/vnd.github+json", "Content-Type": "application/json", "User-Agent": "kesher-master-supervisor-v2"})
            try:
    ...
    def _ensure_ref(self, ref: str) -> None:
        ...
        try:
            self._request("POST", f"{self.api}/git/refs", {"ref": f"refs/heads/{ref}", "sha": sha}, mutation=True, cas_conflict=True)
    ...
    def load_state_blob(self, ref: str, path: str) -> tuple[dict[str, Any] | None, str | None]:
        ...
        payload = self._request("GET", f"{self.api}/contents/{quoted_path}?ref={urllib.parse.quote(ref, safe='')}", allow_404=True)
        if payload is None:
            return None, None
    ...
    def save_state_blob(self, ref: str, path: str, state: dict[str, Any], expected_sha: str | None) -> str:
        self._ensure_ref(ref)
        quoted_path = urllib.parse.quote(path, safe="/")
        body: dict[str, Any] = {
            "message": f"state: Kesher Master Supervisor {state.get('updated_at') or ''}",
            "content": base64.b64encode((json.dumps(state, ensure_ascii=False, indent=2, sort_keys=True) + "\n").encode("utf-8")).decode("ascii"),
            "branch": ref,
        }
        if expected_sha:
            body["sha"] = expected_sha
        payload = self._request("PUT", f"{self.api}/contents/{quoted_path}", body, mutation=True, cas_conflict=True)
    ...
    def workflow_run_by_id(self, run_id: int | str) -> dict[str, Any] | None:
        payload = self._request("GET", f"{self.api}/actions/runs/{run_id}", allow_404=True)
    ...
    def check_runs(self, sha: str) -> list[dict[str, Any]]:
        payload = self._request("GET", f"{self.api}/commits/{sha}/check-runs?per_page=100")
        rows = payload.get("check_runs") if isinstance(payload, dict) else []
        return [row for row in (rows or []) if isinstance(row, dict)]

    def merge_pr(self, number: int, sha: str) -> dict[str, Any]:
        payload = self._request("PUT", f"{self.api}/pulls/{number}/merge", {"sha": sha, "merge_method": "merge", "commit_title": f"Master recovery: merge PR #{number}"}, mutation=True)
        return payload if isinstance(payload, dict) else {}
...
class JulesRecoveryClient:
    ...
    def list_exact(self, fingerprint: str) -> list[dict[str, Any]]:
        ...
        for _ in range(50):
            ...
            payload = jules.request_json("GET", url, jules.jules_headers(self.api_key))
            rows = payload.get("sessions") if isinstance(payload, dict) else []
            matches.extend(row for row in (rows or []) if isinstance(row, dict) and str(row.get("title") or "") == title)
            next_token = str(((payload or {}).get("nextPageToken") or "") if isinstance(payload, dict) else "").strip()
            if not next_token:
                return matches
    ...
    def get(self, session_id: str) -> dict[str, Any]:
        ...
        payload = jules.request_json("GET", f"{jules.API_BASE}/sessions/{sid}", jules.jules_headers(self.api_key))
        return payload if isinstance(payload, dict) else {}
    ...
    def acquire_or_continue(self, packet: dict[str, Any]) -> tuple[str, str]:
        ...
        if len(exact) == 1:
            ...
            if str(row.get("state") or "UNKNOWN").upper() not in self.TERMINAL:
                jules.send_message(self.api_key, name.removeprefix("sessions/"), self._prompt(packet))
                return name, "continued"
            return name, "terminal_existing"
        payload = {
            "title": self.title(fingerprint),
            "prompt": self._prompt(packet),
            "sourceContext": {"source": jules.SOURCE, "githubRepoContext": {"startingBranch": "main"}},
            "requirePlanApproval": False,
            "automationMode": "AUTO_CREATE_PR",
        }
        try:
            created = jules.request_json("POST", f"{jules.API_BASE}/sessions", jules.jules_headers(self.api_key), payload, max_attempts=1)
        except Exception:
...
def reconcile_active_command(state: dict[str, Any], report: dict[str, Any], *, api: Any, jules_client: JulesRecoveryClient | None, now: str) -> tuple[dict[str, Any], bool, str]:
    result = copy.deepcopy(state)
    fp = incident_fingerprint(report)
    ...
    if stage in {"long_video", "short"} and hasattr(api, "active_external_media_run"):
        ...
        if isinstance(external, dict):
            ...
            if should_hold_external_running(started, now):
                return mark_command_running(result, command_id, {"external_run_id": external.get("id"), "external_workflow": external.get("name")}, at=now), False, "external_media_running_heartbeat_exception"
        ...
        if executor == "direct":
            ...
            if prior_pr and hasattr(api, "get_pr"):
                ...
                if isinstance(pr, dict) and pr.get("merged") is True:
                    ...
                    controller_run = api.find_dispatched_run("kesher-content-controller.yml", merged_at) if hasattr(api, "find_dispatched_run") else None
                    if controller_run:
                        return _run_terminal_result(result, command_id, controller_run, "kesher-content-controller.yml", now)
                    ...
                    if age is not None and age >= 1 and hasattr(api, "dispatch_workflow"):
                        ...
                        return mark_command_acknowledged(result, command_id, {"workflow": "recovery_pr_merge", "recovery_pr_number": prior_pr, "controller_dispatched_after_merge": True}, at=now), False, "reconciled_merged_pr_and_woke_controller"
        age = _minutes_since(str(command.get("issued_at") or ""), now)
        if age is not None and age >= UNCERTAIN_ISSUE_GRACE_MINUTES:
            return mark_command_failed(result, command_id, "issued command had no discoverable workflow run after grace period; not blindly reissuing", at=now), True, "uncertain_dispatch_timed_out"
    ...
    if executor == "jules":
        ...
        if not session_id and jules_client:
            ...
            if len(exact_sessions) == 1:
                ...
                return mark_command_failed(result, command_id, "duplicate exact recovery sessions", at=now), True, "duplicate_jules_sessions"
        if session_id and jules_client:
            ...
            if state_name not in JulesRecoveryClient.TERMINAL:
                return mark_command_running(result, command_id, {"session_id": session_id}, at=now), False, "jules_running"
            urls = _pr_urls(session)
            pr_number = _pr_number_from_url(urls[0]) if urls else None
            result = mark_command_failed(result, command_id, f"Jules state={state_name}; exact incident persisted", at=now)
            if pr_number:
        ...
        age = _minutes_since(str(command.get("issued_at") or ""), now)
        if age is not None and age >= UNCERTAIN_ISSUE_GRACE_MINUTES:
            return mark_command_failed(result, command_id, "issued Jules command had no exact session after grace period; not blindly creating another", at=now), True, "uncertain_jules_create_timed_out"
...
def _find_prior_recovery_pr(state: dict[str, Any], fp: str) -> int | None:
    candidates = [row for row in state.get("commands", {}).values() if isinstance(row, dict) and row.get("fingerprint") == fp and row.get("executor") == "jules" and isinstance(row.get("metadata"), dict) and row["metadata"].get("recovery_pr_number")]
    if not candidates:
        return None
...
def _safe_recovery_pr_scope(files: list[str]) -> bool:
    if not files or len(files) > 12:
        return False
    forbidden_prefixes = ("src/data/posts.json", "public/images/generated/blog/", ".kesher-")
...
def _persist_after_side_effect(store: SupervisorStateStore, state: dict[str, Any]) -> None:
    ...
    for key, value in state.get("commands", {}).items():
        ...
        if not isinstance(current, dict) or str(value.get("updated_at") or "") >= str(current.get("updated_at") or ""):
            fresh.setdefault("commands", {})[key] = value
    for key, value in state.get("incidents", {}).items():
        current = fresh.get("incidents", {}).get(key)
        if not isinstance(current, dict) or str(value.get("last_seen_at") or "") >= str(current.get("last_seen_at") or ""):
...
def run_live(*, repo: str, token: str, jules_api_key: str) -> dict[str, Any]:
    ...
    state, resolved_changed = resolve_absent_incidents(state, report, at=observed_at)
    if report.get("status") != "incident_detected":
        if resolved_changed:
            try:
                state_sha = store.save(state, expected_sha=sha)
            except SupervisorCasConflict:
                return {**report, "master_action": "cas_conflict_noop", "execute_now": False}
            return {**report, "master_action": "resolved_absent_incident", "execute_now": False, "state_sha": state_sha}
    ...
    state, terminal, reconcile_reason = reconcile_active_command(state, report, api=api, jules_client=jules_client, now=observed_at)
    state, decision = prepare_escalation(state, report, now=observed_at, prior_action_terminal=terminal)
    try:
        persisted_sha = store.save(state, expected_sha=sha)
    except SupervisorCasConflict:
        return {**report, "master_action": "cas_conflict_noop", "reconcile_reason": reconcile_reason, "execute_now": False}
    if not decision.get("execute_now"):
        return {**report, "master_action": decision.get("stage"), "command_id": decision.get("command_id"), "execute_now": False, "reconcile_reason": reconcile_reason, "state_sha": persisted_sha}
    after = execute_command(state, report, decision, api=api, jules_client=jules_client, now=_now())
    ...
    return {
        **report,
        ...
        "marker": _marker(report, int(command.get("strike") or 0), str(decision.get("stage"))),
    }
...
def main() -> int:
    ...
    if not token:
        ...
        return 2
    try:
        result = run_live(repo=repo, token=token, jules_api_key=str(os.environ.get("JULES_API_KEY") or "").strip())
