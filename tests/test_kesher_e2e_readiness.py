...
class KesherE2EReadinessTests(unittest.TestCase):
    ...
    def test_live_e2e_test_entrypoint_is_isolated_and_requires_three_verified_links(self) -> None:
        ...
        self.assertIn("RUN_LIVE_E2E", workflow)
        self.assertIn("kesher-article-generation.yml", workflow)
        self.assertIn("test_mode=true", workflow)
        self.assertNotIn("test_mode=false", workflow)
        self.assertNotIn(".kesher-controller/state.json", workflow)
        self.assertIn(".kesher-e2e-long-${{ github.run_id }}", workflow)
        self.assertIn(".kesher-e2e-short-${{ github.run_id }}", workflow)
        self.assertIn("production_main_article_unchanged: true", workflow)
        self.assertIn("production_controller_state_untouched: true", workflow)
        self.assertIn("article_preview_url", workflow)
        ...
        self.assertIn("1080", workflow)
        self.assertIn("1920", workflow)
        self.assertNotIn("gh pr merge", workflow)
        self.assertNotIn("auto-merge-article-prs.yml", workflow)
